 ...
 * una interfaz de usuario intuitiva para gestionar productos, pedidos y
 * facturas.
 * 
...
public class SistemaRestauranteGUI extends JFrame {
    ...
    /**
     * Labels para mostrar las estadisticas del sistema.
     * Permiten actualizar los contadores en tiempo real.
     */
    private JLabel lblProductosCount;
    private JLabel lblPedidosCount;
    private JLabel lblFacturasCount;
     ...
     * Muestra estadisticas generales y proporciona acceso rapido a las principales
     * funciones.
     * 
    ...
    private JPanel crearPanelEstadisticas() {
        ...
        lblProductosCount = new JLabel("0");
        lblProductosCount.setForeground(Color.BLUE);
        lblProductosCount.setFont(new Font("Arial", Font.BOLD, 14));

        JLabel lblPedidos = new JLabel("Pedidos Activos:");
        lblPedidosCount = new JLabel("0");
        lblPedidosCount.setForeground(Color.ORANGE);
        lblPedidosCount.setFont(new Font("Arial", Font.BOLD, 14));

        JLabel lblFacturas = new JLabel("Facturas Generadas:");
        lblFacturasCount = new JLabel("0");
        lblFacturasCount.setForeground(Color.GREEN);
        lblFacturasCount.setFont(new Font("Arial", Font.BOLD, 14));
    ...
    private JPanel crearPanelBotonesInicio() {
        ...
        // Configurar estilo del boton de modo cliente
        btnModoCliente.setBackground(new Color(52, 152, 219));
        btnModoCliente.setForeground(Color.WHITE);
        btnModoCliente.setFont(new Font("Arial", Font.BOLD, 12));
        ...
        btnModoCliente.addActionListener(e -> abrirInterfazCliente());

        panelBotones.add(btnActualizar);
        panelBotones.add(btnProductos);
        panelBotones.add(btnPedidos);
        panelBotones.add(btnModoCliente);
    ...
    private void actualizarEstadisticas() {
        if (lblProductosCount != null) {
            int totalProductos = gestorProductos.getTotalProductos();
            lblProductosCount.setText(String.valueOf(totalProductos));
        }

        if (lblPedidosCount != null) {
            int totalPedidosPendientes = gestorPedidos.getTotalPedidosPendientes();
            lblPedidosCount.setText(String.valueOf(totalPedidosPendientes));
        }

        if (lblFacturasCount != null) {
            int totalFacturas = gestorFacturas.getTotalFacturas();
            lblFacturasCount.setText(String.valueOf(totalFacturas));
        }
    }
     ...
     * Permite crear nuevos pedidos, visualizar pedidos existentes y generar
     * facturas.
     * 
    ...
    private void configurarTablaPedidos() {
        String[] columnas = { "ID", "Cliente", "Total", "Estado", "Productos" };
     ...
     * Solicita al usuario los datos necesarios y valida la entrada usando el
     * GestorProductos.
    ...
    private void mostrarDialogoAgregarComida() {
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                if (nombre.isEmpty()) {
                    JOptionPane.showMessageDialog(this, "El nombre no puede estar vacio.", "Error",
                ...
                if (precio <= 0) {
                    JOptionPane.showMessageDialog(this, "El precio debe ser positivo.", "Error",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                JOptionPane.showMessageDialog(this, "Precio invalido. Use numeros decimales.", "Error",
                        JOptionPane.ERROR_MESSAGE);
        ...
        if (result == JOptionPane.OK_OPTION) {
            try {
                ...
                if (descuento < 0 || descuento > 100) {
                    JOptionPane.showMessageDialog(this, "El descuento debe estar entre 0 y 100%.", "Error",
                            JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                JOptionPane.showMessageDialog(this, "Descuento invalido. Use numeros decimales.", "Error",
                        JOptionPane.ERROR_MESSAGE);
            } catch (IllegalArgumentException e) {
                JOptionPane.showMessageDialog(this, e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
     ...
     * Permite al usuario seleccionar productos de la lista disponible usando el
     * GestorPedidos.
     ...
     * Valida que el pedido este pendiente y solicita el nombre del cliente usando
     * el GestorFacturas.
    ...
    private void generarFacturaDesdeSeleccion() {
        ...
        // Obtener el pedido para verificar si ya tiene nombre de cliente
        java.util.Optional<Pedido> pedidoOpt = gestorPedidos.buscarPedidoPorId(idPedido);
        String nombreClienteExistente = "";
        if (pedidoOpt.isPresent()) {
            nombreClienteExistente = pedidoOpt.get().getNombreCliente();
        }

        // Si el pedido ya tiene nombre de cliente, usarlo como valor por defecto
        String cliente;
        if (nombreClienteExistente != null && !nombreClienteExistente.isEmpty()) {
            cliente = nombreClienteExistente;
            ...
            cliente = JOptionPane.showInputDialog(this, "Nombre del cliente:");
        ...
        try {
            ...
            Factura factura = gestorFacturas.generarFactura(idPedido, cliente);
            actualizarTablaPedidos();
     ...
     * Formatea la informacion de las facturas para su visualizacion usando el
     * GestorFacturas.
    ...
    /**
     * Abre una nueva ventana con la interfaz de cliente para realizar pedidos.
     * Permite que multiples clientes hagan pedidos simultaneamente.
     */
    private void abrirInterfazCliente() {
        SwingUtilities.invokeLater(() -> {
            new ClientePedidoGUI(gestorProductos, gestorPedidos);
        });
    }
     ...
     * Sigue las mejores practicas de Swing para la ejecucion de interfaces
     * graficas.
     * 
